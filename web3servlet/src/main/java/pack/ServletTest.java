...
public class ServletTest extends HttpServlet {
	private static final long serialVersionUID = 1L;
       ...
       private ServletTest2 test2; 
   
    public ServletTest() {
        super();
        System.out.println("서블릿 생명주기(lifectcle) : 1  -ServletTest");
    }

	public void init(ServletConfig config) throws ServletException {
		// 서블릿 파일 요청시 최초1회만 수행 : 초기화를 담당
		test2 = new ServletTest2("홍길동");
		
		System.out.println("서블릿 생명주기(lifectcle) : 2  -init");
	}
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 별도 작성한 클래스 멤버 호출
		// ServletTest2 test2 = new ServletTest2("홍길동"); 객체가 요청할 때 마다 생성되므로 메로리 낭비가 심함
		// ServletTest2 test2 = new ServletTest2("홍길동");
		out.print("<br>이름은" + test2.getirum());
		...
		System.out.println("서블릿 생명주기(lifectcle) : 3  -service");
		...
		private int calcData(int num1, int num2) {	// 사용자가 작성한 클래스의 멤버 메소드. 1회만 수행되도록 해야함
			return num1 + num2;
		}
		...
		protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
			...
			System.out.println("서블릿 생명주기(lifectcle) : 4  -doget");
		...
		protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
			...
			System.out.println("서블릿 생명주기(lifectcle) : 4  -doPost");
		...
		public void destroy() {
			// 웹 서버 서비스가 종료되면 수행
			System.out.println("서블릿 생명주기(lifectcle) : 5  -destroy");
