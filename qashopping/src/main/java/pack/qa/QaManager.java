...
public class QaManager {
    ...
    public void saveData(QaBean bean) {
        String sql = "INSERT INTO qa (writer, postpassword, title, postcontent, postcreatedate, secretYN, adminid, replycontent, replyday) "
                   + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = ds.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
    ...
    public QaDto getReplyData(String publishNo) {
        String sql = "SELECT * FROM qa WHERE publish_no=?";
        QaDto dto = null;
    ...
    public void saveEdit(QaBean bean) {
        String sql = "UPDATE qa SET writer=?, postpassword=?, title=?, postcontent=?, postcreatedate=?, secretYN=?, adminid=?, replycontent=?, replyday=? WHERE publish_no=?";
        ...
        try {
            ...
            pstmt.setString(8, bean.getReplycontent());
            pstmt.setString(9, bean.getReplyday());
            pstmt.setInt(10, bean.getPublish_no());
